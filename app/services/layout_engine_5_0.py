...
def _layout_hero_split_6040(slide_data: Dict[str, Any], background: str) -> List[Dict[str, Any]]:
    ...
    objects.append(
        {
            ...
            "children": [
                {"role": "zone_name", "text": slide_data.get("zone_name") or "Donnee non disponible (TBD)", "style": {"font_size": 64, "font_weight": 800, "color": "var(--primary-color)"}},
                {"role": "verdict", "text": slide_data.get("verdict_label") or "Donnee non disponible (TBD)", "style": {"font_size": 28, "font_weight": 600, "color": "var(--secondary-color)"}},
                {"role": "score", "text": slide_data.get("score_label") or "Donnee non disponible (TBD)", "style": {"font_size": 18, "font_weight": 500, "color": "var(--text-muted)"}},
    ...
    metrics = metrics[:6]
    # calcul dynamique de card_height pour que les 6 KPIs tiennent dans le canvas.
    ...
    for idx, metric in enumerate(metrics, start=1):
        ...
        objects.append(
            _kpi_card(
                ...
                width=right_width,
                height=card_height,
                label=metric.get("label") or "Donnee non disponible (TBD)",
                value=metric.get("value") or "Donnee non disponible (TBD)",
...
def _layout_sidebar_analysis_7030(slide_data: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    # Narrative text (exec_summary, competitive_insight, etc.)
    ...
    objects.append(
        {
            ...
            "width": right_width,
            "height": 40,
            "text": slide_data.get("source_label") or "Donnee non disponible (TBD)",
...
def _layout_grid_asymmetric_3columns(slide_data: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not columns:
        columns = [{"label": "Donnee non disponible (TBD)", "value": "Donnee non disponible (TBD)"} for _ in range(3)]
    columns = columns[:3]
    ...
    for idx, column in enumerate(columns, start=1):
        ...
        objects.append(
            _kpi_card(
                ...
                width=col_width,
                height=CANVAS_HEIGHT - content_top - SAFE_MARGIN,
                label=column.get("label") or "Donnee non disponible (TBD)",
                value=column.get("value") or "Donnee non disponible (TBD)",
    ...
    for idx, (pos, quadrant) in enumerate(zip(positions, quadrants), start=1):
        ...
        objects.append(
            _kpi_card(
                ...
                label=quadrant.get("label") or "Donnee non disponible (TBD)",
                value=quadrant.get("value") or "Donnee non disponible (TBD)",
    ...
    for idx, step in enumerate(steps, start=1):
        objects.append(
            _kpi_card(
                ...
                label=step.get("label") or "Donnee non disponible (TBD)",
                value=step.get("value") or "Donnee non disponible (TBD)",
...
def build_slide_5_0(section_id: str, slide_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Clamp de securite : retirer tout objet qui deborde hors canvas.
    safe_objects = []
    for obj in objects:
        ...
        if bottom > CANVAS_HEIGHT or right > CANVAS_WIDTH:
            logger.warning(
                "[layout_engine] objet %s retire (debordement) bottom=%d right=%d section=%s",
